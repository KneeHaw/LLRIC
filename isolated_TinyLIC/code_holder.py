...
class TinyLIC(nn.Module):
    ...
    def __init__(self, N=128, M=320, args=None):
        ...
        self.llric_blk = LRVQ(rank=4, img_size=self.image_size, n_hid=256, embedding_dim=self.patch_size, n_embed=8192, emb_ps=False)
    ...
    def forward(self, x):
        ...
        patches_llric = self.patch_tensor_llric(x) #patches_from_tensor(x, self.patch_size)

        # print(patches_tiny.shape, patches_llric.shape)

        llric_t = time.perf_counter_ns()
        lr_recons, latent_loss, _ = self.llric_blk(patches_llric)
